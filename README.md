# ⚽ 풋살 동호회 앱

동호회 일정·참석응답·당일 출석체크·**실력 기반 공평한 팀 배분**·푸시 알림을 한 곳에서.
PWA(웹앱)라 카카오톡으로 링크만 공유하면 누구나 **홈 화면에 추가**해서 앱처럼 씁니다.

## 기술 스택

- **Next.js 16** (App Router, PWA)
- **Supabase** (인증 + Postgres DB)
- **Web Push** (VAPID) — 푸시 알림
- 배포: **Vercel** (무료)

---

## 처음 한 번만: 설정 순서

### 1) Supabase 프로젝트 만들기

1. https://supabase.com 가입 → **New project** 생성 (Region: Northeast Asia (Seoul) 권장)
2. 좌측 **SQL Editor → New query** 에 `supabase/schema.sql` 내용을 통째로 붙여넣고 **Run**
   - 테이블·보안정책(RLS)·가입 트리거가 한 번에 생성됩니다.
   - 이미 운영 중인 DB에 새 기능(ELO·회비·경기결과 등)을 반영할 때도 **그대로 다시 Run** 하면 됩니다. (`if not exists` / `alter ... add column if not exists` 로 작성되어 재실행해도 안전)
3. **Authentication → Sign In / Providers → Email** 에서
   - 빠른 사용을 원하면 **"Confirm email" 을 끄세요** (가입 즉시 로그인 가능).
   - 메일 인증을 쓰려면 켜둔 채로 둡니다.
4. (선택) **카카오 로그인** 을 쓰려면:
   - [Kakao Developers](https://developers.kakao.com) → 앱 생성 → **REST API 키** 발급, **카카오 로그인 활성화**
   - Redirect URI 에 Supabase 콜백 `https://<프로젝트>.supabase.co/auth/v1/callback` 등록
   - Supabase **Authentication → Sign In / Providers → Kakao** 를 켜고 REST API 키·시크릿 입력
   - 배포 후 Supabase **Authentication → URL Configuration → Redirect URLs** 에 `https://여러분앱.vercel.app/auth/callback` 추가

### 2) API 키를 `.env.local` 에 넣기

`.env.local.example` 을 복사해 `.env.local` 을 만들고, Supabase **Project Settings → API** 값으로 채웁니다.

```
NEXT_PUBLIC_SUPABASE_URL=https://xxxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJ...        # anon public
SUPABASE_SERVICE_ROLE_KEY=eyJ...            # service_role (서버 전용, 절대 노출 금지)
```

VAPID 키는 이미 생성되어 있습니다. (다시 만들려면 `npm run gen:vapid`)

### 3) 로컬 실행

```bash
npm run dev
```

http://localhost:3000 접속 → **회원가입** → 가입 완료.

### 4) 본인을 운영진(admin)으로 지정

운영진만 모임 생성·실력점수 수정·팀 배분이 가능합니다.
Supabase **SQL Editor** 에서 (가입한 이메일로 교체):

```sql
update public.profiles set role = 'admin'
where id = (select id from auth.users where email = 'shcho@wace.me');
```

---

## 배포 (Vercel)

1. 이 폴더를 GitHub 저장소로 푸시
2. https://vercel.com 가입 → **Add New → Project** → 저장소 선택
3. **Environment Variables** 에 `.env.local` 의 값들(SUPABASE 3개 + VAPID 3개) 등록
4. **Deploy** → `https://여러분앱.vercel.app` 주소 생성
5. Supabase **Authentication → URL Configuration → Site URL** 에 배포 주소 등록

### 핸드폰 설치 (PWA)

- 배포 주소를 카카오톡 등으로 공유
- **안드로이드(Chrome)**: 접속 → 메뉴 → "홈 화면에 추가"
- **iOS(Safari)**: 접속 → 공유 → "홈 화면에 추가"
  - ⚠️ iOS는 **홈 화면에 추가한 뒤** 앱을 열어야 푸시 알림이 동작합니다 (Safari 16.4+).

---

## 주요 기능 사용법

| 기능 | 설명 |
|------|------|
| **모임 만들기** | 운영진이 홈에서 `+ 새 모임 만들기` → 자동으로 전원에게 푸시 알림 |
| **정기 모임 반복** | 모임 생성 시 `정기 반복` 선택 → 매주 같은 요일·시간으로 4/8/12회 일괄 생성 |
| **참석 응답** | 모임 상세에서 참석/미정/불참 선택 |
| **대기자 명단** | 정원을 정한 모임은 정원 초과 응답자가 자동으로 `대기자`로. 확정자가 취소하면 응답 순서대로 자동 승격 |
| **출석 체크** | 당일 본인 계정에서 `출석 체크하기` 탭 |
| **팀 배분** | 운영진이 모임 상세에서 `실력 기반 팀 자동 배분` → 출석자(없으면 참석자) 기준으로 점수 합이 균등하게 N팀 편성 |
| **경기 결과·MVP** | 팀 배분 후 운영진이 팀 간 스코어를 기록하고 당일 MVP 지정 |
| **ELO 레이팅** | 경기 결과가 저장될 때마다 멤버 ELO가 자동 재계산. 멤버 탭에서 순위 확인 |
| **실력 점수** | 멤버 탭에서 운영진이 1.0~5.0 조정 (팀 배분 기준, ELO와 별개) |
| **회비 관리** | 회비 탭에서 운영진이 월별 회비 생성 → 멤버별 납부 토글. 멤버는 본인 납부 내역 조회 |
| **카카오 로그인** | 로그인 화면의 `카카오로 시작하기` (Supabase에 Kakao Provider 설정 필요, 아래 참고) |
| **푸시 알림** | 내정보 → `푸시 알림 켜기` |

## 팀 배분 알고리즘

`src/lib/teams.ts` — 실력 점수 내림차순 정렬 후 **스네이크 드래프트**로 초기 배분하고,
인원수가 같은 팀끼리 선수를 **1:1 교환**하며 팀 점수 합 편차를 더 줄입니다 (지역 최적화).
인원은 최대 1명 차이로 맞춰집니다.

## 폴더 구조

```
src/
  app/
    login/page.tsx          로그인·회원가입·카카오 로그인
    auth/callback/route.ts  OAuth(카카오) 콜백 — 코드→세션 교환
    (app)/                  로그인 후 영역 (하단 네비)
      page.tsx              홈 — 모임 목록
      events/new/page.tsx   모임 생성 (운영진) · 정기 반복
      events/[id]/page.tsx  모임 상세 — RSVP·대기자·출석·팀배분·경기결과·MVP
      members/page.tsx      멤버 — ELO 랭킹 / 실력점수 편집
      dues/page.tsx         회비 — 월별 관리(운영진) / 본인 내역(멤버)
      profile/page.tsx      내정보·푸시 설정
    actions.ts              서버 액션 (모든 데이터 변경)
  lib/
    teams.ts                팀 배분 알고리즘
    elo.ts                  경기 결과 기반 Elo 레이팅 계산
    push.ts                 푸시 발송
    supabase/               DB 클라이언트
  proxy.ts                  인증 가드 (Next 16 middleware)
public/
  manifest.json, sw.js      PWA + 서비스워커
supabase/schema.sql         DB 스키마 (재실행 안전)
```

## 확장 기능 (구현됨)

- ✅ 카카오 로그인 (`src/app/auth/callback/route.ts`)
- ✅ 회비/정산 관리 (`/dues`, `dues` 테이블)
- ✅ 경기 결과·MVP 기록 (`matches` 테이블, `events.mvp_user_id`)
- ✅ 경기 결과 기반 자동 레이팅 Elo (`src/lib/elo.ts`, `profiles.elo_rating`)
- ✅ 정기 모임 반복 생성 (`events.series_id`)
- ✅ 대기자 명단 (정원 초과분 자동 처리)

## 다음에 더 확장하면 좋은 것

- 경기별 득점/도움 기록과 개인 시즌 스탯
- 회비 자동 알림(미납 리마인드 푸시), 지출 내역 관리
- 대기자 승격 시 푸시 알림
- 정기 모임 일괄 수정/삭제 (series 단위)
